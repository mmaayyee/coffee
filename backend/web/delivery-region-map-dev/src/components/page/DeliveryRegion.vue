...
export default {
  ...
  methods: {
    ...
    saveRegionData() {
      ...
      axios.post(rootCoffeeUrl+"delivery-api/region-create.html",saveData)
      .then(res=>{
        const data = res.data;
        console.log("保存区域详情..",data);
        if(data.status=="success"){
            const type = this.deliverRegionId===null?"1":"2";
            return axios.get('/activity-combin-package-assoc/create-activity-log?type='+type+'&moduleType=8');
        } else {
            this.alertMsg(data.msg);
